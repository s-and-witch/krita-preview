...
import { readKritaUri } from './kritaFile';
...
export class PreviewManager implements vscode.CustomReadonlyEditorProvider {
	...
	public static readonly viewType = 'kritaPreview.previewEditor';

	private readonly _previews = new Set<KritaPreview>();
	private _activePreview: KritaPreview | undefined;
	...
	public async resolveCustomEditor(
		...
		const preview = new KritaPreview(this.extensionRoot, document.uri, webviewEditor, this.sizeStatusBarEntry, this.binarySizeStatusBarEntry, this.zoomStatusBarEntry);
	...
	private setActivePreview(value: KritaPreview | undefined): void {
		this._activePreview = value;
	}
}


class KritaPreview extends MediaPreview {
...
<head>
	...
	<link rel="stylesheet" href="${escapeAttribute(this.extensionResource('media', 'kritaPreview.css'))}" type="text/css" media="screen" nonce="${nonce}">
...
<body class="container image scale-to-fit loading" data-vscode-context='{ "preventDefaultContextMenuItems": true }'>
	...
	<script src="${escapeAttribute(this.extensionResource('media', 'kritaPreview.js'))}" nonce="${nonce}"></script>
</body>
</html>`;
	}
    ...
    const file = await readKritaUri(resource);
    if(file === undefined) {
      return this.emptyPngDataUri;
    }
		return file;
...
export function registerKritaPreviewSupport(context: vscode.ExtensionContext, binarySizeStatusBarEntry: BinarySizeStatusBarEntry): vscode.Disposable {
	...
	disposables.push(vscode.commands.registerCommand('kritaPreview.zoomIn', () => {
		previewManager.activePreview?.zoomIn();
	}));

	disposables.push(vscode.commands.registerCommand('kritaPreview.zoomOut', () => {
		previewManager.activePreview?.zoomOut();
	}));

	disposables.push(vscode.commands.registerCommand('kritaPreview.copyImage', () => {
